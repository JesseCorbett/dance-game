...
export async function convertFromSM(inputFile: File): Promise<Chart> {
  ...
  const keyPairs = fields
    .map((field) => {
      if (field.startsWith("NOTES")) {
        return [field.substring(0, 5), field.substring(6, field.length - 1)];
      } else {
        return field.split(":");
      }
    })
  ...
  const chart: Chart = {
    ...
    cdCoverUrl: urlIfy(rootPath, fieldValue(keyPairs, "CDTITLE")),
...
function parseLevels(keyPairs: string[][]): ChartLevel[] {
  const notesGroups = keyPairs
    .filter((pairs) => pairs[0] === "NOTES")
    .map((pair) => pair[1]);

  const levels: ChartLevel[] = notesGroups.map((group) => {
    const fields = group.split(":").map((field) => field.trim());
